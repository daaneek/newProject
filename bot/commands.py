...
async def shit(message: types.Message):
    """ Свалка """
    await message.reply('Нажмите /start', parse_mode=types.ParseMode.MARKDOWN)
...
def register_commands(dp: Dispatcher):
    ...
    dp.register_message_handler(start, CommandStart(
        deep_link=re.compile(r'\d+')) | Command("start"), state='*')

    # Меню словарей
    dp.register_message_handler(menu_dictionary, Text(
        equals="📊 Словари") | Command('dict'))
    # Словарь
    dp.register_callback_query_handler(
        show_dictionary, dictionary_button.filter())
    # Мусор
    dp.register_message_handler(
        shit, content_types=types.ContentType.ANY, state='*')
